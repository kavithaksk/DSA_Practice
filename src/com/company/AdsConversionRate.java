...
public class AdsConversionRate {
    ...
    private static void findAdsConversionRate(String[] completed_purchase_user_ids, String[] ad_clicks, String[] all_user_ips) {
        ...
        HashMap<String, List<String>> ipAdTextMap = new HashMap<>();
        ...
        for (int i = 0; i < all_user_ips.length; i++) {
            String[] ipUserIdTokens = all_user_ips[i].split(",");
            if (!ipUserIdMap.containsKey(ipUserIdTokens[1]))
                ipUserIdMap.put(ipUserIdTokens[1], ipUserIdTokens[0]);
        }

        for (int i = 0; i < ad_clicks.length; i++) {
            String[] ipAdTextTokens = ad_clicks[i].split(",");
            String adText = ipAdTextTokens[2];
            String ipAddress = ipAdTextTokens[0];
            if (!ipAdTextMap.containsKey(adText)) {
                List<String> ipAddresses = new ArrayList<>();
                ipAddresses.add(ipAddress);
                ipAdTextMap.put(adText, ipAddresses);
            } else {
                List<String> ipAddresses = ipAdTextMap.get(adText);
                ipAddresses.add(ipAddress);
                ipAdTextMap.put(adText, ipAddresses);
            }
        }

        for (Map.Entry<String, List<String>> kv : ipAdTextMap.entrySet()) {
            String adText = kv.getKey();
            int totalNumberofAdClicks = kv.getValue().size();
            int validNoOfClicks = 0;
            for (String s : kv.getValue()) {
                String userId = ipUserIdMap.getOrDefault(s, null);
                if (userId != null && completedPurchaseUserIds.contains(userId))
                    validNoOfClicks++;
            }
            System.out.println(validNoOfClicks + " of " + totalNumberofAdClicks + " " + adText);
        }
    }
